...
CACHE_PAGE_URL_FILE = "./downloads/cache/cache_page_urls.json"
CACHE_IMG_URL_FILE = "./downloads/cache/cache_image_urls.json"
...
def download_images(url_list: list, output_dir: str = DOWNLOAD_PATH) -> None:
            ...
            with session.get(url, stream=True, verify=False, timeout=30, headers=headers) as response:
                if response.status_code == 200:  # 首次下载或服务器不支持Range
                    if start_byte > 0:  # 如果之前有下载部分，但服务器不支持Range，则重新下载
                        ...
                        mode = "wb"
                        print(
                            f"The server does not support resuming from breakpoint or re-downloading if the file has been updated: {filename}"
                        )
                    ...
                    if content_range:
                        ...
                        if match:
                            ...
                            if range_start != start_byte:
                                print(
                                    f"Warning: The starting byte of the request does not match the server response. Download again: {filename}"
                                )
                                start_byte = 0
                                mode = "wb"
    ...
    with ThreadPool(processes=URL_FETCHING_THREADS) as pool:
        ...
        results = list(
            tqdm(pool.imap_unordered(download_file, url_list), total=len(url_list), desc="Downloading images")
        )
        # 仍然使用tqdm显示进度
